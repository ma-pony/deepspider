...
```
deepspider/
├── bin/cli.js               # CLI 入口（config/version/update/mcp）
├── src/
│   ├── mcp/                 # MCP Server
...
│   │   │   ├── NetworkInterceptor.js   # 网络请求自动记录
│   │   │   ├── ScriptInterceptor.js    # JS 脚本自动记录
│   │   │   └── AntiDebugInterceptor.js # 反调试保护
...
│   ├── core/                # 核心模块
│   │   ├── Sandbox.js       # isolated-vm 沙箱
│   │   └── PatchGenerator.js # 补丁代码生成
│   ├── env/                 # 环境补丁模块
│   │   └── modules/         # 各 API 的补环境代码生成器
│   ├── cli/                 # CLI 命令层
│   └── config/              # 配置（paths.js + settings.js）
├── .claude/commands/ds/     # Slash commands（trace/reverse/rebuild/crawl）
├── skills/deepspider/       # References（领域知识文档）
└── test/                    # 测试
```
...
使用 `@babel/traverse`（非 acorn-walk），`traverse.default(ast, visitors)` 模式。
...
1. 在 Claude Code 中使用 `/ds:trace https://target-site.com` 开始追踪
2. 浏览器自动打开，在网站上操作触发目标请求
3. 使用 `/ds:reverse` 分析加密逻辑并生成 Python 实现
4. 如遇 VM 混淆，使用 `/ds:rebuild` 导出补环境项目
5. 使用 `/ds:crawl` 生成完整 Python 爬虫

## Active Technologies
- Node.js 18+ (ESM), Python 3.10+ (爬虫产物) + `@opencode-ai/sdk`, `opencode-ai`, `gray-matter`, `patchright`, `isolated-vm`, `@modelcontextprotocol/sdk`, `zod` (001-opencode-agent)
- 文件系统 (`~/.deepspider/output/`, `~/.deepspider/db/`, `~/.deepspider/config.json`) (001-opencode-agent)

## Recent Changes
- 001-opencode-agent: Added Node.js 18+ (ESM), Python 3.10+ (爬虫产物) + `@opencode-ai/sdk`, `opencode-ai`, `gray-matter`, `patchright`, `isolated-vm`, `@modelcontextprotocol/sdk`, `zod`
