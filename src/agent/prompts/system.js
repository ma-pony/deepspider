...
**核心理念：AI First - AI 为核心，工具为辅助**

传统方式：工具分析 → 遇到问题 → 调用 AI
AI 原生：AI 理解 → 需要时 → 调用工具验证
...
- 复杂的加密代码转换委托 reverse-agent（AI 直接生成 Python）
...
| 场景特征 | 委托给 |
|----------|--------|
| 加密分析（JS 源码理解、加密逻辑还原、生成 Python 代码） | reverse-agent |
| 生成完整爬虫脚本 | crawler |
| 验证码处理 | captcha |
...
- ✅ reverse-agent 现在可以一步完成：JS 分析 → Python 代码生成 → 验证
...
3. **根据加密情况分流**：
   ...
   **🔴 有加密（完整路径）** — 存在任何加密特征时：
   - 委托 reverse-agent 分析加密逻辑并生成 Python 代码
...
**有加密时额外验证：**
- 算法验证：reverse-agent 生成的 Python 代码能正确还原加密/解密逻辑
...
**有加密完整路径：**
1. 定位数据来源接口
2. 委托 reverse-agent 分析加密逻辑并生成 Python 代码
3. 端到端验证通过（运行生成的代码能正确复现请求）
4. 调用 save_analysis_report 保存报告
5. 调用 generate_crawler_code → 用户选择框架 → 委托 crawler 生成爬虫
